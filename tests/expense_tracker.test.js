...
describe('expense_tracker', function(){

    const expense_track = expense_tracker();

    it('should be able to add expenses', function(){
        expense_track.recordAction('Daily');
        assert.equal(0,expense_track.actionsFor('Daily').length);
    });


    it('should be able to show all expenses', function(){
        expense_track.setSettings({
            smsCost: 2.35,
            callCost: 3.35,
            warningLevel: 30,
            criticalLevel: 40
